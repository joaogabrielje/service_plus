...
interface RegisterFormProps {
  isAdminForm?: boolean; // Para saber se é o admin criando usuário
}

export function RegisterForm({ isAdminForm = false }: RegisterFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Se é admin criando usuário, incluir orgId e role
      if (isAdminForm && session?.user) {
        bodyData.orgId = (session.user as any).orgId;
        bodyData.role = role;
      }
      ...
      if (response.ok) {
        if (isAdminForm) {
          // Se é admin, redirecionar para página de usuários e recarregar
    ...
    <Card className="w-full max-w-md">
      <CardHeader className="text-center">
        <CardTitle className="text-2xl">
          {isAdminForm ? "Cadastrar Usuário" : "Criar Conta"}
        </CardTitle>
        <CardDescription>
          {isAdminForm 
            ? "Cadastre um novo usuário para sua organização" 
            : "Cadastre-se com suas informações ou utilize o Google"
          }
        </CardDescription>
      </CardHeader>
      <CardContent>
